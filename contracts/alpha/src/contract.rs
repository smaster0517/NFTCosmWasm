...
use crate::{CudosMsgWrapper};

use crate::error::ContractError;
use crate::msg::{create_issue_denom_msg, ExecuteMsg, InstantiateMsg, QueryMsg};
use crate::query::{DenomResponse};
...
pub fn execute(
    ...
    match msg {
        ExecuteMsg::MsgIssueDenom {
            id ,
            name ,
            schema,
            sender
        } => execute_msg_issue_denom(deps, _env, info, id, name, schema, sender),
    }
}
...
pub fn query(deps: Deps, _env: Env, msg: QueryMsg) -> StdResult<Binary> {
    match msg {
        QueryMsg::Denom {
            denom_id
        } => to_binary(&query_denom(deps, denom_id)?),
    }
}
